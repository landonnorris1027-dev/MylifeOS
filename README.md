...
Focus and break sessions also persist their wall-clock deadline in native Preferences. This avoids timer drift while the WebView is paused and restores the session after the process restarts. When notifications are enabled, Android keeps a native completion alarm scheduled in both foreground and background, so WebView/process loss does not remove the reminder. Notification permission is requested in the foreground. Exact alarm access is checked before scheduling; without it, Android uses an inexact alert, which may arrive late. Grant alarm/reminder access in Android settings for the most timely delivery. Declining permissions does not stop the in-app timer.

The focus settings include **Vibrate when ringing** (响铃时震动), enabled by default for existing and new installations. On the visible timer screen, Android plays a short system notification sound paired with two 200 ms vibrations separated by 150 ms. The native receiver owns automatic completion alerts; the page does not replay them. Turning off foreground sound also disables its paired vibration. Background notifications follow the notification setting and Android channel/silent/Do Not Disturb policy, using separate sound-only and sound-with-vibration channels. Replacement channels inherit the old channel’s importance, sound and vibration restrictions; subsequent user edits to the new channels are preserved. Settings are captured when a session starts. Reopening an expired timer does not replay its alert. Manual early completion alerts; pause, cancel, and skipping a break do not. Vibration is Android-only.

...
- `app-data.json`、`recovery-points.json`、`pomodoro-state.json`、`window-state.json` 和用户导出的 JSON 备份统一改为“同目录临时文件 → 刷盘 → 原子替换”，并保留上一份完整内容为 `.bak`。
- 启动时主文件损坏会回退读取 `.bak`；防抖写入失败会回滚内存状态，并通过主进程事件在界面显示保存失败，不再静默丢失。
- 故障注入覆盖半截临时写、`fsync` 失败、损坏主文件回退、延迟写失败通知；另用 Windows 真实文件系统验证已有文件可被原子替换且旧版本完整保留。
- 正常退出继续同步冲刷 300 ms 防抖队列。强制结束或系统崩溃仍可能丢失尚未进入落盘阶段的最近操作，但不会把上一次已落盘 JSON 覆盖成半截。

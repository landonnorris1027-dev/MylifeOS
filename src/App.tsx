...
import { initializeDay, updateTask, getTodayStr, deleteTaskFromDay, reduceHabitQuota, formatDateLocal, parseDateLocal } from './services/storage';
...
export default function App() {
    ...
    const restorePomodoroState = async () => {
      try {
        ...
        if (activeTimers.length > 0) {
          // 如果有活动的计时器，可以显示通知或处理相应逻辑
          console.log('发现活动的番茄钟:', activeTimers);
          // 这里可以根据需要添加UI提示逻辑
        }
      } catch (error) {
  ...
  const handleScheduleConfirm = (time: string) => {
    if (schedulingTask) {
      // --- Duration Conflict Check ---
      // 逻辑修正：计算该时段内所有任务的总时长，防止超过 60 分钟
      const existingTasksInSlot = (dailyData?.tasks || []).filter(
      ...
      const newTaskDuration = schedulingTask.durationMinutes;

      if (currentTotalDuration + newTaskDuration > 60) {
        setAlertConfig({
          isOpen: true,
          message: t('duration_limit_exceeded', {
            time: time,
            current: currentTotalDuration,
            new: newTaskDuration
          })
        });
